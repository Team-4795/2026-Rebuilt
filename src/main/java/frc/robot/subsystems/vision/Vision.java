...
public class Vision extends SubsystemBase {
    ...
    public void periodic() {
       for (int i = 0; i < io.length; i++) {
          ...
          Logger.recordOutput("Vision/" + VisionConstants.CAM_NAMES[i] + "/is updating", isVisionUpdating());
        }
      ...
      for (int i = 0; i < io.length; i++) { // Iterate through all cams
          for (int p = 0; p < inputs[i].pose.length; p++) { // Iterate through estimated poses
              ...
              double xyStdDev = (tagPoses.size() == 1 ? VisionConstants.XY_SINGLE_STDEV : VisionConstants.XY_MULTIPLE_STDEV) * Math.pow(distance, 2);
